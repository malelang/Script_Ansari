...
%         plot(ecg1)
%         hold all;
%         plot(pks2,ecg1(pks2),'o');
%         
%         close(h);
        
        %widths = find_peak_width(ecg1,pks,0.5);
...
%             end
        ...
        dlmwrite([data_dir alarm '\Detected_old\' fname '.ann'],detected);
        %count_FN
        if(count_FN>1)
            asdf=1;
        end
...
function width = find_peak_width(sig,pk_locs,height_ratio)

    width = zeros(size(pk_locs));

    idx = find(sig(1:pk_locs(1))<height_ratio*sig(pk_locs(1)),1,'last');
    idx2 = find(sig(pk_locs(1):pk_locs(2))<height_ratio*sig(pk_locs(1)),1,'first');
    idx2 = idx2 + pk_locs(1) - 1;
    ...
    for i = 2:length(pk_locs)-1
        idx = find(sig(pk_locs(i-1):pk_locs(i))<height_ratio*sig(pk_locs(i)),1,'last');
        idx = idx + pk_locs(i-1) - 1;
        idx2 = find(sig(pk_locs(i):pk_locs(i+1))<height_ratio*sig(pk_locs(i)),1,'first');
        idx2 = idx2 + pk_locs(i) - 1;
    ...
    idx = find(sig(pk_locs(end-1):pk_locs(end))<height_ratio*sig(pk_locs(end)),1,'last');
    idx = idx + pk_locs(end-1) - 1;
    idx2 = find(sig(pk_locs(end):end)<height_ratio*sig(pk_locs(end)),1,'first');
    idx2 = idx2 + pk_locs(end) - 1;
