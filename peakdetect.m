function [R_i,R_amp,S_i,S_amp,T_i,T_amp,Q_i,Q_amp]=peakdetect(ecg,fs)
    ...
    %% Locate the R wave location by finding the highest local maxima
      if state == 1 % look for the highest peak
            ...
            if  currentmax > buffer_plot(i-window)
                ...
                % Locate Q wave
                [Q_tamp Q_ti] = min(buffer_plot(max(ind-round(0.040*fs),1):(ind))); % modified by Sardar
                Q_ti = max(ind-round(0.040*fs),1) + Q_ti -1;
                Q_i = [Q_i Q_ti];
                Q_amp = [Q_amp Q_tamp];
      ...
      %% Enter S wave detection state3 (S detection)
       ...
       if state ==6   
         ...
         if c <= 0.7*fs  
             ...
             % two threshold
             if(isempty(S_amp1)) % modified by Sardar
                 thres2 = ((abs(abs(buffer_T)-abs(0)))*3/4 + 0); 
             else
