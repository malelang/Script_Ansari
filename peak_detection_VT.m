function pk_locs = peak_detection_VT( ecg1_orig, ecg2_orig, abp_orig, ppg_orig, Fs, max_freq )   
...
%            ppg=ppg';
    ...
    %%%%%%%%%%%%%%ESTO ESTABA COMENTADO DESDE EL PRINCIPIO
...
%%%%%%%%%%%%%%% SE QUITA ESTA LINEA CUANDO SE HACE SIN FILTRO 
    ...
    [~,init_pks] = findpeaks(abs(ecg1),'MinPeakDistance',Fs);
    if(median(abs(ecg1(init_pks)))>0)%(prctile(ecg1,99)-prctile(ecg1,1))>0)        
        %ecg1 = ecg1./(prctile(ecg1,99)-prctile(ecg1,1));
    ...
    [~,init_pks] = findpeaks(abs(ecg2),'MinPeakDistance',Fs);
    if(median(abs(ecg2(init_pks)))>0)%(prctile(ecg2,99)-prctile(ecg2,1))>0)        
        %ecg2 = ecg2./(prctile(ecg2,99)-prctile(ecg2,1));
        ecg2 = ecg2./median(abs(ecg2(init_pks)));
    end
    
    %abp = abp - medfilt1(abp,Fs);
    [~,init_pks] = findpeaks(abs(abp),'MinPeakDistance',Fs);
    if(median(abs(abp(init_pks)))>0)%(prctile(abp,99)-prctile(abp,1))>0)
        abp = abp./median(abs(abp(init_pks)));
    end
    
    %ppg = ppg - medfilt1(ppg,Fs);
    [~,init_pks] = findpeaks(abs(ppg),'MinPeakDistance',Fs);
    if(median(abs(ppg(init_pks)))>0)%(prctile(ppg,99)-prctile(ppg,1))>0)
...
%     close(h);   
    ...
    ecg1_indic = indicator(ecg1,ecg1_pks1,ecg1_pks1_q,Fs) + indicator(ecg1,ecg1_pks2,ecg1_pks2_q,Fs) + indicator(ecg1,ecg1_pks4,ecg1_pks4_q,Fs);
    ecg2_indic = indicator(ecg2,ecg2_pks1,ecg2_pks1_q,Fs) + indicator(ecg2,ecg2_pks2,ecg2_pks2_q,Fs) + indicator(ecg2,ecg2_pks4,ecg2_pks4_q,Fs);        
...
%    ppg_indic = ppg_indic1 + ppg_indic2;% + ppg_indic3;
    ...
    [~,pk_locs] = findpeaks(indic,'MinPeakDistance',round(Fs/max_freq),'MinPeakHeight',1.5,'MinPeakProminence',1.5);
...
function [pk_locs, pk_q] = my_ecg_peak_detection(sig, Fs, max_freq, st_sum)
    ...
    [~,pk_locs,w] = findpeaks(der','MinPeakDistance',round(Fs/max_freq),'MinPeakProminence',depth_th,'WidthReference','halfheight');
...
function [pk_locs, pk_q] = sqrs_peak_detection(sig,Fs,max_freq,st_sum)
    ...
    sig2 = [zeros(1,1000), sig];

    count = 0;
    while(count < 5)
        try
            out = evalc(['mat2wfdb(sig2'',''ecg' id ''',Fs,[],''v'')']);
            count = 5;
        catch
            count = count + 1;
...
function pk_locs = search_neighborhood(sig, pk_locs, span)
    ...
    pk_trs = zeros(length(pk_locs)+1,1);
    [~,idx] = min(sig2(1:pk_locs(1)-1));
    pk_trs(1) = idx;    
    for i = 2:length(pk_locs)
        [~,idx] = min(sig2(pk_locs(i-1)+1:pk_locs(i)-1));
        pk_trs(i) = idx + pk_locs(i-1);
    end
    [~,idx] = min(sig2(pk_locs(end)+1:end));
    pk_trs(end) = idx + pk_locs(end);    
    
    pk_heights = mean([sig2(pk_locs)-sig2(pk_trs(1:end-1)) ; sig2(pk_locs)-sig2(pk_trs(2:end))]);
    
    if(sum(pk_heights < 0.2*prctile(pk_heights,60))>0)
        fasd=1;
    end
    
    pk_locs(pk_heights < 0.2*prctile(pk_heights,60)) = [];    
end
